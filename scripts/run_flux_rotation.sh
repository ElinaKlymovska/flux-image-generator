...
# This script generates character images from different rotation angles using FLUX 1.1 [pro] API
...
show_usage() {
    ...
    echo "  -k, --api-key KEY               FLUX API key (required)"
    echo "  -t, --test-connection           Test API connection before generation"
    echo "  -h, --help                      Show this help message"
    echo ""
    echo "Examples:"
    echo "  $0 -k YOUR_API_KEY"
    echo "  $0 -k YOUR_API_KEY -a 45 90 135 -s 999999"
    echo "  $0 -k YOUR_API_KEY -i /path/to/character.jpg -o my_rotations"
}
...
if [[ -z "$API_KEY" ]]; then
    print_error "API key is required. Use -k or --api-key option."
    show_usage
    exit 1
fi
...
PYTHON_SCRIPT="$BIN_DIR/generate_flux_rotation.py"
...
# Print configuration
print_info "FLUX 1.1 [pro] Character Rotation Generator"
print_info "=========================================="
