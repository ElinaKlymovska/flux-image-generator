...
# Run batch Adetailer generation
print_status "Starting batch Adetailer image generation..."
python bin/generate_adetailer_batch.py

if [ $? -eq 0 ]; then
    print_success "Batch Adetailer generation completed successfully!"
    print_status "Check the data/output directory for generated images"
else
    print_error "Batch Adetailer generation failed!"
    exit 1
fi 